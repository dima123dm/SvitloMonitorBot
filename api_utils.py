    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                PRIMARY_API_URL, timeout=aiohttp.ClientTimeout(total=15)
            ) as response:
...
async def fetch_backup_api():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                BACKUP_API_URL, timeout=aiohttp.ClientTimeout(total=15)
            ) as response:
                if response.status == 200:
        ...
        for queue_id, p_queue_data in p_schedule.items():
            ...
            for date_str, date_data in b_queue_data.items():
                if date_str not in p_queue_data:
                    ...
                    print(
                        f"[merge] {p_name}/{queue_id}: добавлено дату {date_str} з backup"
                    )
    ...
    if api_state["active_source"] == "primary":
        ...
        if primary_data:
            ...
            if elapsed >= FAILOVER_TIMEOUT:
                ...
                print(
                    f"🔄 FAILOVER: Перемикання на РЕЗЕРВНЕ API! (Основне не працює {elapsed/3600:.1f} год)"
                )
        ...
        if api_state["last_primary_check"]:
            elapsed_since_check = (
                now - api_state["last_primary_check"]
            ).total_seconds()
            ...
            if api_state["backup_down_since"]:
                ...
                if elapsed_backup >= FAILOVER_TIMEOUT:
                    ...
                    if primary_data is None:
                        print(
                            "🔄 FAILOVER: Резервне теж не працює 2+ год! Пробуємо основне..."
                        )
                        primary_data = await fetch_primary_api()
                    if primary_data:
                        api_state["active_source"] = "primary"
...
def get_api_status():
    ...
    result = {
        ...
        "active_source_name": (
            "🟢 DTEK Proxy (Основне)"
            if state["active_source"] == "primary"
            else "🟡 Резервне API"
        ),
        "total_switches": state["total_switches"],
        "last_switch": (
            state["last_switch"].strftime("%d.%m %H:%M")
        ...
        for element in post_div.children:
            ...
            if element.name == "ul" and current_date_str:
                for li in element.find_all("li"):
                    parse_queue_line(
                        li.get_text(strip=True), current_date_str, schedule_map
                    )
...
def calculate_on_hours(schedule_data):
    ...
    if not schedule_data:
        return 24.0  # Якщо даних немає (пусто), вважаємо що світло є
...
def format_message(
    schedule_json, queue_name, date_str, is_tomorrow=False, display_mode="blackout"
):
